...
| 10 | Supervisor restart-on-crash via real subprocess lifecycle | `tests/test_supervisor.py` | `culture/clients/claude/supervisor.py` — 93% standalone (lines 87, 90-92, 114, 145 still uncovered, mostly error branches); same shape for codex (38%), copilot (37%), acp (45%) supervisors | claude: 93% standalone vs. ≈42% integration / **+51 pp**; non-claude backends drop from ≈40% to **0%** integration-only | **ADD integration** (Task 7) — but **NOTE**: integration test should be parameterized over all four backends (Plan Task 7 currently only mentions claude — see "Recommendations for plan updates" below) |
...
- **ADD integration** rows: 1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14 → **13 rows mapping to Plan Tasks 2–8** (with one row, #10, suggesting a Plan-Task-7 expansion; see Recommendations).
- **ACCEPT** rows: 9, 16, 17, 18, 19, 21 → **6 rows**.
...
2. **Task 7: parameterize over all 4 backends, not just claude.** ✅ **APPLIED IN THIS PR** (see plan Task 7 — now uses a `BACKEND_MODULES` dict mirroring Task 8). Original recommendation: The plan's Task 7 (`tests/test_integration_supervisor.py`) currently launches `python -m culture.clients.claude.supervisor` only. Per row #10, the integration-baseline coverage of `culture/clients/{codex,copilot,acp}/supervisor.py` is **0%**. If Task 7 doesn't parameterize, the closeout coverage will show three supervisor.py files at 0% and the gate will fail (or have to be set artificially low). Suggest the test mirror Task 8's `BACKEND_MODULES` pattern. The Phase-1 retargeting note in the plan (line 829) already anticipates four-backend parameterization indirectly; making it explicit here closes the loop.
