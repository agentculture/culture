...
- BREAKING: the `culture agent` (singular) CLI noun is renamed to `culture agents` and the singular noun is removed — `culture agent <verb>` now errors with "invalid choice". All lifecycle verbs moved to `culture agents` (e.g. `culture agents start`, `culture agents status`, `culture agents install`).
- Systemd/launchd units generated by `culture agents install` now use `culture agents start` in ExecStart; the service unit name `culture-agent-<nick>.service` is unchanged. Existing units must be regenerated by re-running `culture agents install <nick>` — see the migration note in docs/reference/cli/agent-systemd.md.
