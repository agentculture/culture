"""Webhook delivery client for bridge daemon alert hooks.

Cite-don't-import copy of ``culture/clients/claude/webhook.py`` —
each backend owns its copy (codex, copilot, acp all do the same).
Bridge maintains this copy to satisfy the architecture rule that
forbids ``culture/clients/<backend>/`` directories from importing
each other's modules (Qodo PR #50 #1, Rule 428332).

Keep in sync with ``culture/clients/claude/webhook.py`` via the
all-backends rule in CLAUDE.md: when a feature is added or changed
in any backend's webhook module, propagate to all.
"""
...
from culture.clients.bridge.config import WebhookConfig
