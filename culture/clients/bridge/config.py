"""Backend config dataclasses for the bridge daemon.

Cite-don't-import copy of ``culture/clients/claude/config.py`` —
each backend (claude / codex / copilot / acp / bridge) owns its
own copy so per-backend tweaks stay local and the directories do
not cross-import. Qodo PR #50 #1 (Rule 428332).

Most bridge deployments don't exercise the LLM-only fields
(``model``, ``permission_mode``, ``system_prompt`` etc.) — the
bridge is a transport-only daemon — but the dataclass surface
stays identical so a future shared-config consolidation does not
become a breaking change for the bridge's yaml schema. Keep in
sync with ``culture/clients/claude/config.py`` via the
all-backends rule in CLAUDE.md.
"""
