"""Chat subcommands: culture chat {start,stop,status,default,rename,archive,unarchive}.

Renamed from ``culture server`` in culture 9.0.0 — the new noun fits the
mesh's purpose better (it's an agent chat mesh, not a generic server).
The 7 verbs above stay culture-owned; everything else (``restart``,
``link``, ``logs``, ``version``, ``serve``, plus any new verb agentirc
adds in future versions) falls through verbatim to
``agentirc.cli.dispatch`` so it lands under ``culture chat <verb>`` for
free.

The 7 culture-owned verbs are NOT forwarded because:

- ``start`` embeds ``agentirc.ircd.IRCd`` in-process and registers
  culture's ``BotManager`` against it (per Phase A2). Forwarding to
  ``agentirc start`` would lose the bot registration — bots would never
  load.
...
The legacy ``culture server`` noun stays available through 9.x via
``culture/cli/server.py`` as a deprecation alias that prints a warning
and routes to this module.
"""
...
def register_verbs(
    ...
    *,
    ...
    """Register the culture-chat verb tree under ``parser_name``.

    Factored out so the legacy ``culture server`` deprecation alias
    (``culture/cli/server.py``) can register the identical subparser
    tree under ``server`` without duplicating the verb definitions.
    Both ``chat`` and ``server`` resolve to the same handlers via
    :func:`dispatch_verb`.
    """
    server_parser = subparsers.add_parser(parser_name, help=parser_help)
    server_sub = server_parser.add_subparsers(dest=dest_attr)
...
def register(subparsers: argparse._SubParsersAction) -> None:
    register_verbs(
        subparsers,
        parser_name="chat",
        parser_help="Manage the IRC chat mesh",
        dest_attr="chat_command",
    )
...
def dispatch_verb(
    ...
    *,
    ...
    """Run ``verb`` against the culture-owned handlers, or forward.

    ``noun`` is just the user-facing label (``chat`` or ``server``)
    used in the missing-verb usage string and the unknown-verb error.
    Both nouns route through the same handlers — only the noun in the
    help text changes.
    """
    if not verb:
        print(
            f"Usage: culture {noun} "
    ...
    if handler is None:
        print(f"Unknown {noun} command: {verb}", file=sys.stderr)
        sys.exit(1)
    handler(args)


def dispatch(args: argparse.Namespace) -> None:
    dispatch_verb(getattr(args, "chat_command", None), args, noun="chat")
...
def _server_rename(args: argparse.Namespace) -> None:
    ...
    if server_running:
        ...
        print(f"  culture chat stop --name {new_name}")
        print(f"  culture chat start --name {new_name}")
    if renamed:
...
def _check_server_archived(args: argparse.Namespace) -> None:
    ...
    if config.server.name == args.name and config.server.archived:
        ...
        print(f"  culture chat unarchive --name {args.name}", file=sys.stderr)
        sys.exit(1)
...
def _server_archive(args: argparse.Namespace) -> None:
    ...
    print(f"\nTo restore: culture chat unarchive --name {server_name}")
...
def _server_unarchive(args: argparse.Namespace) -> None:
    ...
    print(f"\nStart with: culture chat start --name {server_name}")
