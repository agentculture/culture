...
Also pins `culture_core.cli.shared.formatting` as a re-export of
`culture_core.formatting.relative_time` since that's a sub-10-line module
...
from culture_core.cli import afi, devex
...
def test_afi_dispatch_forwards_argv(monkeypatch):
    captured: list = []
    monkeypatch.setattr(
        "culture_core.cli._passthrough.run",
...
# ---------------------------------------------------------------------------
# culture_core.cli.shared.formatting — trivial re-export
# ---------------------------------------------------------------------------


def test_cli_shared_formatting_reexports_relative_time():
    from culture_core.cli.shared import formatting
    from culture_core.formatting import relative_time as canonical
