"""Tests for `culture_core.cli.shared.mesh` — link/mesh helpers for CLI.
...
filesystem fixtures and `monkeypatch` for `culture_core.credentials.lookup_credential`.
...
from culture_core.cli.shared import mesh as mesh_mod
...
class TestResolveLinksFromMesh:
    def test_returns_empty_when_no_peers(self, monkeypatch):
        monkeypatch.setattr(
            "culture_core.mesh_config.load_mesh_config",
            lambda _p: _StubMesh(server=_StubServer(links=[])),
        )
        monkeypatch.setattr("culture_core.credentials.lookup_credential", lambda _n: "unused")
    ...
    def test_builds_one_link_per_peer_with_keyring_password(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "culture_core.credentials.lookup_credential",
...
class TestBuildServerStartCmd:
    ...
    def test_skips_link_with_invalid_peer_name(self, monkeypatch, caplog):
        """A malformed link name in mesh.yaml must not crash server start —
        the link is skipped with a warning (Qodo review, PR #24)."""
        peers = [
            _StubLink(name="bad name!", host="bad.example.com", port=6667),
            _StubLink(name="thor", host="thor.example.com", port=6667),
        ]
        monkeypatch.setattr(
            "culture_core.mesh_config.load_mesh_config",
        ...
        def _lookup(name):
            from culture_core.credentials import _validate_peer_name

            _validate_peer_name(name)
            return "thor-pw"

        monkeypatch.setattr("culture_core.credentials.lookup_credential", _lookup)

        with caplog.at_level("WARNING", logger="culture"):
            links = mesh_mod.resolve_links_from_mesh("/tmp/mesh.yaml")

        assert [link.name for link in links] == ["thor"]
        assert any(
            "Skipping link 'bad name!'" in r.message and "mesh.yaml" in r.message
