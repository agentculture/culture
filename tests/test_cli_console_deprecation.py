...
def test_mesh_console_warns_then_forwards(capsys):
    ...
    The forwarded call uses `culture_core.cli.console.dispatch_resolved_argv`,
    ...
    with patch("culture_core.cli.mesh.console_dispatch") as forwarded:
...
def test_mesh_console_help_marks_deprecated():
    result = subprocess.run(
        [sys.executable, "-m", "culture_core", "mesh", "--help"],
        capture_output=True,
        text=True,
        check=False,
    )
