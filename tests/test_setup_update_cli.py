...
from culture_core.mesh_config import MeshAgentConfig, MeshConfig, MeshLinkConfig, MeshServerConfig
...
def test_update_parser():
    ...
    # default upgrade-timeout matches the constant in culture_core.cli.mesh
    from culture_core.cli.mesh import _UPGRADE_TIMEOUT_SECONDS
...
def test_setup_in_dispatch():
    ...
    from culture_core.cli import mesh
...
_MESH_MOD = "culture_core.cli.mesh"
...
@patch("culture_core.pidfile.list_servers")
...
def test_update_discovers_running_servers(
    ...
    from culture_core.cli.mesh import _cmd_update
...
@patch("culture_core.pidfile.list_servers", return_value=[])
@patch("culture_core.mesh_config.load_mesh_config")
@patch(f"{_MESH_MOD}._restart_mesh_services")
@patch(f"{_MESH_MOD}._upgrade_culture_package", return_value=True)
...
def test_run_upgrade_streams_output():
    ...
    """
    from culture_core.cli.mesh import _run_upgrade
...
def test_run_upgrade_timeout_message_has_three_hints(capsys):
    """On timeout, the remediation must offer all three recovery paths.
    ...
    hint = exc_info.value.remediation
    assert "uv tool upgrade culture" in hint  # direct-tool suggestion
    assert "--upgrade-timeout" in hint  # extend-timeout suggestion
    assert "--skip-upgrade" in hint  # skip suggestion
...
def test_run_upgrade_timeout_message_uses_pip_when_pip_selected(capsys):
    ...
    hint = exc_info.value.remediation
    assert "pip install --upgrade culture" in hint
    assert "uv tool upgrade culture" not in hint
...
def test_resolve_uses_mesh_yaml_when_name_matches(tmp_path):
    ...
    from culture_core.cli.mesh import _resolve_mesh_for_server
    ...
    from culture_core.mesh_config import save_mesh_config
...
def test_install_mesh_services_omits_legacy_config_path():
    ...
    # culture_core.persistence import install_service`), so patch the source
    # module rather than the attribute on culture_core.cli.mesh.
    with (
...
def test_resolve_rebuilds_from_agents_yaml_preserving_links(tmp_path):
    ...
    with patch(f"{_MESH_MOD}.DEFAULT_CONFIG", str(tmp_path / "agents.yaml")):
        from culture_core.clients.claude.config import save_config
