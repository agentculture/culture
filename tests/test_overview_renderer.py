...
def test_partial_snapshot_line_when_rooms_failed():
    """Issue #16: a failed room is surfaced as an explicit partial-snapshot note."""
    from culture_core.overview.model import FailedRoom

    mesh = _make_fixture()
    mesh.failed_rooms = [FailedRoom(name="#broken", error="TimeoutError", phase="WHO")]
    output = render_text(mesh)
    assert "**Partial snapshot:** 2 of 3 rooms collected" in output
    assert "failed: #broken (TimeoutError during WHO)" in output
    # Collected rooms still render.
    assert "## #general" in output
    assert "## #dev" in output


def test_partial_snapshot_lists_every_failed_room():
    """Issue #16: multiple failed rooms are all named."""
    from culture_core.overview.model import FailedRoom

    mesh = _make_fixture()
    mesh.failed_rooms = [
        FailedRoom(name="#broken", error="TimeoutError", phase="WHO"),
        FailedRoom(name="#garbled", error="ValueError", phase="HISTORY"),
    ]
    output = render_text(mesh)
    assert "2 of 4 rooms collected" in output
    assert "#broken (TimeoutError during WHO)" in output
    assert "#garbled (ValueError during HISTORY)" in output


def test_no_partial_snapshot_line_when_all_rooms_collected():
    """Issue #16: complete snapshots render without any partial marker."""
    mesh = _make_fixture()
    output = render_text(mesh)
    assert "Partial snapshot" not in output


def test_room_drilldown_failed_room_reports_failure():
    """Issue #16: drilling into a failed room explains the failure, not 'not found'."""
    from culture_core.overview.model import FailedRoom

    mesh = _make_fixture()
    mesh.failed_rooms = [FailedRoom(name="#broken", error="ValueError", phase="HISTORY")]
    output = render_text(mesh, room_filter="#broken")
    assert "Room #broken failed to collect (ValueError during HISTORY)." in output
