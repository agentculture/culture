...
from culture_core.cli.shared.display import (
    _fetch_ipc_data,
...
def test_agent_process_status_stopped():
    """No PID file → stopped."""
    agent = _make_agent()
    with patch("culture_core.cli.shared.display.read_pid", return_value=None):
...
def test_agent_process_status_running(tmp_path):
    ...
    sock.touch()
    with (
        patch("culture_core.cli.shared.display.read_pid", return_value=12345),
        patch("culture_core.cli.shared.display.is_process_alive", return_value=True),
        patch("culture_core.cli.shared.display.agent_socket_path", return_value=str(sock)),
    ):
        status, pid = agent_process_status(agent)
...
def test_agent_process_status_starting(tmp_path):
    ...
    with (
        patch("culture_core.cli.shared.display.read_pid", return_value=12345),
        patch("culture_core.cli.shared.display.is_process_alive", return_value=True),
        patch("culture_core.cli.shared.display.agent_socket_path", return_value=str(missing_sock)),
    ):
        status, pid = agent_process_status(agent)
...
def test_fetch_ipc_data_success():
    ...
    with (
        patch("culture_core.cli.shared.display.agent_socket_path", return_value="/tmp/test.sock"),
        patch("culture_core.cli.shared.display.ipc_request", return_value=fake_resp),
    ):
...
def test_fetch_ipc_data_failure():
    ...
    with (
        patch("culture_core.cli.shared.display.agent_socket_path", return_value="/tmp/test.sock"),
        patch("culture_core.cli.shared.display.ipc_request", return_value=None),
    ):
        data = _fetch_ipc_data(agent)
    assert data is None
...
def _mock_running_agent_overview(agents, ipc_data, capsys, show_activity=False):
    ...
    with (
        patch("culture_core.cli.shared.display.read_pid", return_value=999),
        patch("culture_core.cli.shared.display.is_process_alive", return_value=True),
        patch("culture_core.cli.shared.display.agent_socket_path", return_value="/tmp/x.sock"),
        patch("os.path.exists", return_value=True),
        patch("culture_core.cli.shared.display.ipc_request", return_value=ipc_data),
    ):
...
def test_print_bot_listing_hides_archived_by_default(tmp_path, monkeypatch, capsys):
    ...
    from culture_core.bots import config as bots_config
    from culture_core.cli.shared.display import print_bot_listing
...
def test_bot_list_also_skips_malformed_entries(tmp_path, monkeypatch):
    ...
    from culture_core.bots import config as bots_config
    from culture_core.cli.bot import _load_and_filter_bots
