"""Tests for `culture_core.cli.bot` — culture bot {create,start,stop,list,inspect,archive,unarchive}.

Each handler is invoked directly with an `argparse.Namespace`. The bot
filesystem lives at `culture_core.bots.config.BOTS_DIR` (a `~/.culture/bots`
...
from culture_core.cli import bot as bot_mod
from culture_core.cli._errors import CultureError
from culture_core.cli.shared.constants import BOT_CONFIG_FILE
...
def bots_dir(monkeypatch, tmp_path):
    """Redirect culture_core.bots.config.BOTS_DIR to a tmp_path."""
    d = tmp_path / "bots"
    d.mkdir()
    monkeypatch.setattr("culture_core.bots.config.BOTS_DIR", d)
    return d
...
def stub_config(monkeypatch):
    """Patch `culture_core.config.load_config_or_default` to return a deterministic stub."""
    cfg = _StubConfig()
...
class TestDispatch:
    ...
    def test_dispatch_unknown_command(self, capsys):
        ...
        assert exc.value.code == 1
        assert "Unknown bot command" in exc.value.message
        assert "culture bot --help" in exc.value.remediation
...
class TestBotCreate:
    ...
    def test_rejects_empty_name(self, bots_dir, stub_config, capsys):
        ...
        assert exc.value.code == 1
        assert "bot name cannot be empty" in exc.value.message
    ...
    def test_rejects_duplicate(self, bots_dir, stub_config, capsys):
        ...
        assert "culture bot inspect spark-ori-ghci" in exc.value.remediation
...
class TestBotStartStop:
    def test_start_unknown_bot_exits(self, bots_dir, capsys):
        ...
        assert "culture bot list" in exc.value.remediation
...
class TestBotUnarchive:
    ...
    def test_not_archived_exits(self, bots_dir, capsys):
        ...
        assert "culture bot list --all" in exc.value.remediation
...
class TestPathTraversalGuard:
    ...
    def test_create_rejects_dotdot_name(self, bots_dir, stub_config, capsys):
        ...
        assert exc.value.code == 1
        assert "invalid bot name" in exc.value.message
