"""Tests for `culture_core.telemetry.audit` rotation + error paths.
...
from culture_core.telemetry import init_audit
...
from culture_core.telemetry.audit import reset_for_tests as _reset_audit
...
class TestWriteAll:
    ...
    def test_short_write_loops_until_complete(self, monkeypatch):
        ...
        monkeypatch.setattr("culture_core.telemetry.audit.os.write", _short_write)
    ...
    def test_zero_byte_write_raises_oserror(self, monkeypatch):
        ...
        monkeypatch.setattr("culture_core.telemetry.audit.os.write", _stuck)
...
class TestSubmitBeforeStart:
    def test_submit_before_start_drops_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level("WARNING", logger="culture_core.telemetry.audit"):
            sink.submit({"k": "v"})
    ...
    async def test_shutdown_closes_fd_swallowing_oserror(self, tmp_path, monkeypatch):
        ...
        def _close_raise(fd):
            if fd == sink._current_fd:
                ...
                monkeypatch.setattr("culture_core.telemetry.audit.os.close", original_close)
                original_close(fd)
                raise OSError("simulated close failure")
            return original_close(fd)

        monkeypatch.setattr("culture_core.telemetry.audit.os.close", _close_raise)
...
class TestRotation:
    ...
    async def test_maybe_rotate_open_failure_keeps_old_fd_open(self, tmp_path, monkeypatch, caplog):
        ...
        with patch.object(sink, "_open_audit_file", side_effect=OSError("EACCES"), autospec=True):
            with caplog.at_level("ERROR", logger="culture_core.telemetry.audit"):
