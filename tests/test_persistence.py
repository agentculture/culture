...
def test_build_systemd_unit():
    ...
    assert "[Unit]" in unit
    assert "Description=culture-core server spark" in unit
    assert f"-m culture_core server start" in unit
    assert "--name spark" in unit
    assert "Restart=on-failure" in unit
    assert "WantedBy=default.target" in unit


def test_build_systemd_unit_parks_permanent_failures():
    """Transient crashes self-heal (Restart=on-failure + RestartSec) but a
    permanent daemon failure (exit 78, the #15 contract) parks the unit in a
    clear failed state instead of restart-looping every 5 seconds forever."""
    unit = _build_systemd_unit(
        name="culture-server-spark",
...
def test_build_windows_bat_stops_looping_on_permanent_exit():
    """Parity with systemd's RestartPreventExitStatus (#15): the retry loop
    must stop when the daemon signals a permanent failure (exit 78)."""
    bat = _build_windows_bat(
        command=["culture", "server", "start", "--foreground", "--name", "spark"],
    )
    assert f"if %ERRORLEVEL% EQU {EXIT_DAEMON_PERMANENT} goto end" in bat
    # The permanent-exit check must come before the retry delay re-loops.
    assert bat.index(f"EQU {EXIT_DAEMON_PERMANENT} goto end") < bat.index("timeout /t 5")
...
class TestRunCmd:
    def test_success_returns_true(self):
        ...
        with patch("culture_core.persistence.subprocess.run", return_value=completed):
    ...
    def test_timeout_returns_false(self):
        ...
        with patch("culture_core.persistence.subprocess.run", side_effect=_raise):
...
class TestUninstallService:
    ...
    def test_unsupported_platform_is_noop(self):
        with (
            patch("culture_core.persistence.get_platform", return_value="aix"),
            patch("culture_core.persistence._run_cmd") as mock_run,
        ):
...
class TestListServices:
    ...
    def test_macos_returns_empty_when_no_agent_dir(self, tmp_path):
        ...
        with (
            patch("culture_core.persistence.get_platform", return_value="macos"),
            patch("culture_core.persistence._launchd_dir", return_value=ghost),
        ):
...
class TestRestartService:
    ...
    def test_linux_restart_invokes_systemctl(self, tmp_path):
        ...
        with (
            patch("culture_core.persistence.get_platform", return_value="linux"),
            patch("culture_core.persistence._systemd_user_dir", return_value=unit_dir),
            patch("culture_core.persistence._run_cmd", return_value=True) as mock_run,
        ):
    ...
    def test_macos_unload_failure_returns_false(self, tmp_path):
        ...
        with (
            patch("culture_core.persistence.get_platform", return_value="macos"),
            patch("culture_core.persistence._launchd_dir", return_value=agent_dir),
            patch("culture_core.persistence._run_cmd", return_value=False),
        ):
    ...
    def test_macos_load_after_unload_returns_true(self, tmp_path):
        ...
        with (
            patch("culture_core.persistence.get_platform", return_value="macos"),
            patch("culture_core.persistence._launchd_dir", return_value=agent_dir),
            patch("culture_core.persistence._run_cmd", return_value=True),
        ):
    ...
    def test_windows_query_timeout_returns_false(self):
        ...
        with (
            patch("culture_core.persistence.get_platform", return_value="windows"),
            patch("culture_core.persistence.subprocess.run", side_effect=_raise),
        ):
            assert restart_service("server-spark") is False
    ...
    def test_windows_query_missing_returns_false(self):
        ...
        with (
            patch("culture_core.persistence.get_platform", return_value="windows"),
            patch("culture_core.persistence.subprocess.run", return_value=completed),
        ):
            assert restart_service("ghost") is False
