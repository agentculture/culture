...
from culture_core.cli.channel import (
    _require_ipc,
    _try_ipc,
    _valid_nick,
...
from culture_core.cli.shared.ipc import ipc_request
from culture_core.clients.shared.ipc import encode_message, make_response
...
class TestObserverFallbackWarning:
    ...
    def test_topic_read_does_not_warn(self, monkeypatch, capsys):
        """_topic_read uses _try_ipc but errors on failure — no fallback, no warning.

        Regression guard against the Qodo/Copilot review on PR #304: the
        previous design auto-warned inside `_try_ipc` and printed a
        misleading 'falling back' notice for `topic` (which actually errors).
        """
        from culture_core.cli.channel import _topic_read
        ...
        assert "topic query requires" in exc.value.message
...
class TestRequireIpc:
    ...
    def test_join_requires_culture_nick(self, monkeypatch):
        ...
        with pytest.raises(CultureError) as exc_info:
            _require_ipc("irc_join", channel="#ops")
        assert exc_info.value.code == 1
        assert "CULTURE_NICK must be set" in exc_info.value.message
        assert "export CULTURE_NICK=spark-claude" in exc_info.value.remediation
...
def _stub_ipc(monkeypatch, *, try_resp=None, require_resp=None, capture=None):
    ...
    """
    from culture_core.cli import channel as ch_mod
    ...
    def _fake_require(msg_type, **kwargs):
        ...
        if result is None:
            ...
            raise CultureError(
                1,
                "cannot reach agent daemon",
                "check the agent is running: culture agents status",
            )
        return result
...
class TestIsConnectionError:
    def test_classifies_known_oserror_strings(self):
        from culture_core.cli.channel import _is_connection_error
...
class TestInterpretEscapes:
    def test_passthrough_when_no_backslashes(self):
        from culture_core.cli.channel import _interpret_escapes
...
class TestDispatchWrapsConnectionError:
    def test_handler_oserror_with_connection_message_exits_with_hint(self, monkeypatch, capsys):
        ...
        assert "cannot connect to IRC server" in exc.value.message
    ...
    def test_unknown_command_exits(self, capsys):
        from culture_core.cli.channel import dispatch

        with pytest.raises(CultureError) as exc:
            dispatch(_make_args(channel_command="frobnicate"))
        assert exc.value.code == 1
        assert "Unknown channel command" in exc.value.message
        assert "culture channel --help" in exc.value.remediation


class TestCmdListBody:
    def test_ipc_path_renders_channel_list(self, monkeypatch, capsys):
        from culture_core.cli.channel import _cmd_list
...
class TestCmdReadBody:
    def test_rejects_empty_target(self, capsys):
        from culture_core.cli.channel import _cmd_read

        with pytest.raises(CultureError) as exc:
            _cmd_read(_make_args(channel_command="read", target="   ", limit=50))
        assert exc.value.code == 1
        assert "channel name cannot be empty" in exc.value.message
...
class TestCmdMessageBody:
    def test_rejects_empty_target(self, capsys):
        from culture_core.cli.channel import _cmd_message
    ...
    def test_rejects_text_with_only_escaped_newlines(self, capsys):
        ...
        assert exc.value.code == 1
        assert "no non-empty line" in exc.value.message
    ...
    def test_rejects_nonexistent_channel_without_create(self, monkeypatch, capsys):
        ...
        assert "--create" in exc.value.remediation
...
class TestCmdWho:
    def test_rejects_empty_target(self, capsys):
        from culture_core.cli.channel import _cmd_who
...
class TestCmdJoinPart:
    def test_join_happy_path(self, monkeypatch, capsys):
        from culture_core.cli.channel import _cmd_join
    ...
    def test_join_failure_exits_with_error(self, monkeypatch, capsys):
        ...
        with pytest.raises(CultureError) as exc:
            _cmd_join(_make_args(channel_command="join", target="#ops"))
        assert exc.value.code == 1
        assert "banned" in exc.value.message
        assert "culture agents status" in exc.value.remediation

    def test_part_happy_path(self, monkeypatch, capsys):
        from culture_core.cli.channel import _cmd_part
    ...
    def test_part_failure_exits_with_error(self, monkeypatch, capsys):
        ...
        with pytest.raises(CultureError) as exc:
            _cmd_part(_make_args(channel_command="part", target="#ops"))
        assert exc.value.code == 1
        assert "not in channel" in exc.value.message


class TestCmdAsk:
    def test_rejects_empty_target(self, capsys):
        from culture_core.cli.channel import _cmd_ask
    ...
    def test_rejects_empty_text(self, capsys):
        ...
        assert exc.value.code == 1
        assert "question text cannot be empty" in exc.value.message
    ...
    def test_failure_exits(self, monkeypatch, capsys):
        ...
        with pytest.raises(CultureError) as exc:
            _cmd_ask(_make_args(channel_command="ask", target="#ops", text="?", timeout=30))
        assert exc.value.code == 1
        assert "no answer" in exc.value.message


class TestCmdTopic:
    def test_topic_rejects_empty_target(self, capsys):
        from culture_core.cli.channel import _cmd_topic
    ...
    def test_topic_set_failure_exits(self, monkeypatch, capsys):
        from culture_core.cli.channel import _topic_set

        _stub_ipc(monkeypatch, require_resp={"ok": False, "error": "not op"})
        with pytest.raises(CultureError) as exc:
            _topic_set("#ops", "anything")
        assert exc.value.code == 1
        assert "not op" in exc.value.message
    ...
    def test_topic_read_exits_when_daemon_unreachable(self, monkeypatch, capsys):
        """_topic_read uses _try_ipc but errors on None response (not silent fallback)."""
        from culture_core.cli.channel import _topic_read
...
class TestCmdCompactClear:
    def test_compact_happy_path(self, monkeypatch, capsys):
        from culture_core.cli.channel import _cmd_compact
    ...
    def test_clear_happy_path(self, monkeypatch, capsys):
        from culture_core.cli.channel import _cmd_clear
    ...
    def test_clear_failure_exits(self, monkeypatch, capsys):
        ...
        with pytest.raises(CultureError) as exc:
            _cmd_clear(_make_args(channel_command="clear"))
        assert exc.value.code == 1
        assert "not ready" in exc.value.message
...
class TestRequireIpcDaemonUnreachable:
    def test_require_ipc_exits_when_daemon_response_is_none(self, monkeypatch, capsys):
        ...
        from culture_core.cli.channel import _require_ipc
        ...
        with pytest.raises(CultureError) as exc:
            _require_ipc("irc_join", channel="#ops")
        assert exc.value.code == 1
        assert "cannot reach agent daemon" in exc.value.message
        assert "culture agents status spark-claude" in exc.value.remediation
