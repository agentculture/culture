"""Tests for the shared console helpers extracted from culture_core.cli.mesh."""

from __future__ import annotations

from unittest.mock import patch

from culture_core.cli.shared.console_helpers import resolve_console_nick, resolve_server


def test_resolve_server_returns_none_when_no_servers():
    with patch("culture_core.cli.shared.console_helpers.list_servers", return_value=[]):
        assert resolve_server(None) is None


def test_resolve_server_named_with_known_port():
    with patch("culture_core.cli.shared.console_helpers.read_port", return_value=7000):
...
def test_resolve_server_uses_server_prefixed_pidfile_key():
    ...
    culture_core.cli.server). Looking up the bare `<name>` would always miss
    ...
    with patch("culture_core.cli.shared.console_helpers.read_port", side_effect=fake_read_port):
...
def test_resolve_server_unknown_name_returns_none():
    ...
    """
    with patch("culture_core.cli.shared.console_helpers.read_port", return_value=None):
        assert resolve_server("nope") is None


def test_resolve_server_single_server_picks_it():
    with patch(
        "culture_core.cli.shared.console_helpers.list_servers",
...
def test_resolve_server_default_match_wins():
    ...
    with (
        patch("culture_core.cli.shared.console_helpers.list_servers", return_value=servers),
        patch("culture_core.cli.shared.console_helpers.read_default_server", return_value="b"),
    ):
...
def test_resolve_server_multi_no_default_emits_hint(capsys):
    ...
    with (
        patch("culture_core.cli.shared.console_helpers.list_servers", return_value=servers),
        patch("culture_core.cli.shared.console_helpers.read_default_server", return_value=None),
    ):
...
def test_resolve_console_nick_uses_user_env_when_git_fails():
    ...
    with (
        patch("culture_core.cli.shared.console_helpers.subprocess.run", return_value=fake_run),
...
def test_resolve_console_nick_sanitizes_git_name():
    ...
    with patch("culture_core.cli.shared.console_helpers.subprocess.run", return_value=fake_run):
