"""Tests for `culture_core.cli.server` — `culture server {start,stop,status,default,rename,archive,unarchive}`.
...
- `culture_core.pidfile.{read_pid, write_pid, remove_pid, is_process_alive,
  ...
  rename_pid, list_servers}` — patched on the `culture_core.cli.server` module
  (where they're imported at module top).
- `os.kill`, `time.sleep`, `socket.create_connection`.
- `culture_core.config.{load_config_or_default, rename_manifest_server,
...
def pids(monkeypatch):
    ...
    monkeypatch.setattr(
        "culture_core.pidfile.write_default_server",
...
class TestCmdDefault:
    def test_accepts_running_server(self, monkeypatch, pids, capsys, tmp_path):
        monkeypatch.setattr("culture_core.pidfile.list_servers", lambda: [{"name": "spark"}])
        monkeypatch.setattr("culture_core.pidfile.PID_DIR", str(tmp_path / "no-pids"))
        # Configured server is also accepted; stub it.
        monkeypatch.setattr(
            "culture_core.config.load_config_or_default",
            lambda _p: _StubConfig(),
        )
        # Patch write_default_server inside _cmd_default's import (lazy).
        wrote = []
        monkeypatch.setattr(
            "culture_core.pidfile.write_default_server", lambda name: wrote.append(name)
        )
    ...
    def test_rejects_unknown_server(self, monkeypatch, tmp_path):
        monkeypatch.setattr("culture_core.pidfile.list_servers", lambda: [])
        monkeypatch.setattr("culture_core.pidfile.PID_DIR", str(tmp_path))
        ...
        assert "culture server start --name ghost" in exc.value.remediation

    def test_rejects_unknown_server_lists_known(self, monkeypatch, tmp_path):
        monkeypatch.setattr("culture_core.pidfile.list_servers", lambda: [{"name": "spark"}])
        monkeypatch.setattr("culture_core.pidfile.PID_DIR", str(tmp_path / "no-pids"))
        monkeypatch.setattr(
            "culture_core.config.load_config_or_default",
        ...
        with pytest.raises(CultureError) as exc:
            srv_mod._cmd_default(argparse.Namespace(name="ghost"))
        assert exc.value.code == 1
        assert "known servers: spark" in exc.value.message
        assert "culture server default spark" in exc.value.remediation
...
class TestDispatch:
    ...
    def test_unknown_verb_exits(self, monkeypatch, pids):
        with pytest.raises(CultureError) as exc:
            srv_mod.dispatch(_args(server_command="frobnicate", name="spark"))
        assert exc.value.code == 1
        assert "Unknown server command" in exc.value.message
        assert "culture server --help" in exc.value.remediation
...
class TestMaybeSetDefaultServer:
    def test_writes_when_no_default(self, monkeypatch):
        wrote = []
        monkeypatch.setattr("culture_core.pidfile.read_default_server", lambda: None)
        monkeypatch.setattr(
            "culture_core.pidfile.write_default_server", lambda name: wrote.append(name)
        )
    ...
    def test_does_not_overwrite_existing(self, monkeypatch):
        wrote = []
        monkeypatch.setattr("culture_core.pidfile.read_default_server", lambda: "thor")
        monkeypatch.setattr(
            "culture_core.pidfile.write_default_server", lambda name: wrote.append(name)
        )
...
class TestCheckServerArchived:
    def test_exits_when_archived(self, monkeypatch):
        cfg = _StubConfig(server=_StubServerCfg(name="spark", archived=True))
        monkeypatch.setattr("culture_core.config.load_config_or_default", lambda _p: cfg)

        with pytest.raises(CultureError) as exc:
            srv_mod._check_server_archived(_args(name="spark"))
        assert exc.value.code == 1
        assert "is archived" in exc.value.message
        assert "culture server unarchive --name spark" in exc.value.remediation
...
class TestCheckAlreadyRunning:
    def test_exits_when_running(self, pids):
        ...
        assert "culture server stop --name spark" in exc.value.remediation
...
class TestServerStop:
    def test_exits_when_no_pidfile(self, pids):
        with pytest.raises(CultureError) as exc:
            srv_mod._server_stop(_args(name="spark"))
        assert exc.value.code == 1
        assert "No PID file" in exc.value.message
        assert "culture server start --name spark" in exc.value.remediation
...
class TestValidateConfigName:
    ...
    def test_mismatch_exits(self):
        ...
        assert exc.value.code == 1
        assert "name mismatch" in exc.value.message
        assert "--name other" in exc.value.remediation
...
class TestUpdateSingleBotArchive:
    ...
    def test_archives_active_bot(self, monkeypatch):
        saved = []
        monkeypatch.setattr(
            "culture_core.bots.config.save_bot_config",
    ...
    def test_unarchives_archived_bot(self, monkeypatch):
        monkeypatch.setattr("culture_core.bots.config.save_bot_config", lambda *a: None)
...
class TestSetBotsArchiveState:
    def test_returns_empty_when_bots_dir_missing(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("culture_core.bots.config.BOTS_DIR", ghost)
    ...
    def test_cascades_to_matching_bots(self, monkeypatch, tmp_path):
        from culture_core.bots.config import BotConfig, save_bot_config
        ...
        monkeypatch.setattr("culture_core.bots.config.BOTS_DIR", bots)
...
class TestServerArchive:
    ...
    def test_archives_server_and_cascades(self, monkeypatch, pids, capsys):
        ...
        monkeypatch.setattr("culture_core.config.load_config_or_default", lambda _p: cfg)
        monkeypatch.setattr(
            "culture_core.config.archive_manifest_server",
        ...
        stopped = []
        monkeypatch.setattr("culture_core.cli.shared.process.stop_agent", stopped.append)
...
class TestServerUnarchive:
    def test_not_archived_exits(self, monkeypatch):
        cfg = _StubConfig(server=_StubServerCfg(name="spark", archived=False))
        monkeypatch.setattr("culture_core.config.load_config_or_default", lambda _p: cfg)
        with pytest.raises(CultureError) as exc:
            srv_mod._server_unarchive(_args(name="spark"))
        assert exc.value.code == 1
        assert "is not archived" in exc.value.message
        assert "culture server status --name spark" in exc.value.remediation
    ...
    def test_unarchives_server_and_cascades(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("culture_core.config.load_config_or_default", lambda _p: cfg)
        monkeypatch.setattr(
            "culture_core.config.unarchive_manifest_server", lambda path: ["spark-ada"]
        )
        monkeypatch.setattr(srv_mod, "_set_bots_archive_state", lambda *a, **kw: ["bot1"])
...
class TestServerRename:
    def _setup(self, monkeypatch, *, sanitize_raises=False, rename_raises=None, rename_result=None):
        if sanitize_raises:
            monkeypatch.setattr(
                "culture_core.config.sanitize_agent_name",
                lambda n: (_ for _ in ()).throw(ValueError("bad name")),
            )
        else:
            monkeypatch.setattr("culture_core.config.sanitize_agent_name", lambda n: n)
        if rename_raises is not None:
            monkeypatch.setattr(
                "culture_core.config.rename_manifest_server",
    ...
    def test_rename_manifest_error_exits(self, monkeypatch):
        ...
        assert exc.value.code == 1
        assert "server not found" in exc.value.message
        assert exc.value.remediation
    ...
    def test_renames_pid_and_agents(self, monkeypatch, capsys):
        ...
        pid_renames = []
        monkeypatch.setattr(
            "culture_core.pidfile.rename_pid", lambda old, new: pid_renames.append((old, new))
        )
        monkeypatch.setattr("culture_core.pidfile.read_default_server", lambda: "spark")
        wrote_default = []
        monkeypatch.setattr("culture_core.pidfile.write_default_server", wrote_default.append)
        monkeypatch.setattr("culture_core.pidfile.read_pid", lambda _n: None)
        monkeypatch.setattr("culture_core.pidfile.is_process_alive", lambda _p: False)
    ...
    def test_warns_when_server_still_running(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("culture_core.pidfile.rename_pid", lambda old, new: None)
        monkeypatch.setattr("culture_core.pidfile.read_default_server", lambda: None)
        monkeypatch.setattr("culture_core.pidfile.read_pid", lambda _n: 4242)
        monkeypatch.setattr("culture_core.pidfile.is_process_alive", lambda _p: True)
