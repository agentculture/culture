...
def test_culture_overview_cli_runs():
    result = subprocess.run(
        [sys.executable, "-m", "culture_core", "overview"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_culture_learn_cli_runs():
    result = subprocess.run(
        [sys.executable, "-m", "culture_core", "learn"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_culture_explain_unknown_topic_exits_1():
    result = subprocess.run(
        [sys.executable, "-m", "culture_core", "explain", "unknown-topic-xyz"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_culture_explain_shipped_namespace_returns_real_content():
    ...
    for ns in ("agents", "server", "mesh", "channel", "bot", "skills"):
        result = subprocess.run(
            [sys.executable, "-m", "culture_core", "explain", ns],
            capture_output=True,
            text=True,
            check=False,
        )
...
def _run_cli(*argv: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "culture_core", *argv],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_explain_noun_verb_json_has_argparse_markdown():
    ...
    assert "agents start" in payload["markdown"]
...
def test_emit_result_json_to_stream():
    ...
    from culture_core.cli._output import emit_result
...
def test_emit_diagnostic_to_stream():
    ...
    from culture_core.cli._output import emit_diagnostic
