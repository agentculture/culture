"""Unit tests for `culture_core.cli.console._resolve_argv`.
...
def test_no_running_servers_raises_culture_error_with_hint():
    with (
        ...
        assert "No culture servers running" in excinfo.value.message
        assert "culture server start" in excinfo.value.remediation


def test_unknown_named_server_raises_friendly_culture_error():
    ...
    with (
        ...
        assert "culture server status" in excinfo.value.remediation
