...
def pid_dir(tmp_path):
    ...
    with patch("culture_core.pidfile.PID_DIR", str(tmp_path)):
        yield tmp_path
...
class TestCheckPortConflict:
    ...
    def test_same_target_exits_zero(self, pid_dir, capsys):
        ...
        with (
            patch("culture_core.pidfile.is_culture_process", return_value=True),
    ...
    def test_different_target_exits_one_with_hint(self, pid_dir, capsys):
        ...
        assert excinfo.value.code == 1
        assert "'thor'" in excinfo.value.message
        assert "thor-ada" in excinfo.value.message
        hint = excinfo.value.remediation
        assert "culture console stop --web-port 8765" in hint
        assert "--web-port 8766" in hint
        assert "culture console spark" in hint
    ...
    def test_stale_pidfile_pid_belongs_to_non_culture_process(self, pid_dir):
        ...
        (pid_dir / "console-8765.json").write_text(json.dumps({"server_name": "x"}))
        with (
            patch("culture_core.pidfile.is_culture_process", return_value=False),
            patch.object(console, "_port_in_use", return_value=True),
            patch.object(console, "_looks_like_irc_lens", return_value=False),
        ):
            console._check_port_conflict(
    ...
    def test_foreign_irc_lens_without_pidfile_exits_with_hint(self, pid_dir, capsys):
        ...
        assert excinfo.value.code == 1
        assert "irc-lens" in excinfo.value.message
        assert "wasn't started by `culture console`" in excinfo.value.message
        assert "ss -tlnp" in excinfo.value.remediation
...
class TestCmdStop:
    ...
    def test_non_culture_process_refused_preserves_state(self, pid_dir, capsys):
        ...
        pidfile.write_port(_slot(8765), 8765)
        (pid_dir / "console-8765.json").write_text("{}")
        with patch("culture_core.pidfile.is_culture_process", return_value=False):
            with pytest.raises(CultureError) as excinfo:
                console._cmd_stop(_stop_args(["stop"]))
        assert excinfo.value.code == 1
        assert "refusing to stop" in excinfo.value.message
        assert "manually" in excinfo.value.remediation
    ...
    def test_graceful_stop_via_sigterm(self, pid_dir, capsys):
        ...
        with (
            patch("culture_core.pidfile.is_culture_process", return_value=True),
            patch("culture_core.pidfile.is_process_alive", side_effect=lambda _: next(alive_calls)),
    ...
    def test_force_stop_escalates_to_sigkill(self, pid_dir, capsys):
        ...
        with (
            patch("culture_core.pidfile.is_culture_process", return_value=True),
            patch("culture_core.pidfile.is_process_alive", return_value=True),
            patch("os.kill", side_effect=fake_kill),
    ...
    def test_sigkill_skipped_if_pid_recycled_to_non_culture(self, pid_dir, capsys):
        ...
        with (
            patch(
                "culture_core.pidfile.is_culture_process", side_effect=lambda _: next(culture_calls)
            ),
            patch("culture_core.pidfile.is_process_alive", return_value=True),
            patch("os.kill", side_effect=fake_kill),
            patch.object(console, "_STOP_GRACE_SECONDS", 0.05),
...
class TestDispatchStopRouting:
    def test_stop_short_circuits_before_passthrough(self, pid_dir):
        ...
        with pytest.raises(SystemExit) as excinfo:
            console.dispatch(ns)
        # _cmd_stop returns None (S3516 fix); dispatch runs it then calls
        # sys.exit(0) explicitly (avoids S3699 "use of void return"), so the
        # raised SystemExit carries code=0.
        assert excinfo.value.code == 0
    ...
    def test_empty_console_args_does_not_crash_on_index(self, pid_dir):
        ...
        with patch.object(console._passthrough, "run"):
            # Will try to resolve a default server, which raises CultureError
