"""Tests for `culture_core.cli.shared.process` — agent/server shutdown.
...
`asyncio.run(ipc_shutdown)`, the `culture_core.pidfile.*` helpers) are
...
from culture_core.cli.shared import process as proc_mod
