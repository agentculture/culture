"""Tests for `culture_core.cli.mesh` — `culture mesh {overview,setup,update,console}`.
...
from culture_core.cli import mesh as mesh_mod
...
class _StubFullConfig:
    """Stand-in for culture_core.config.ServerConfig returned by load_config_or_default."""
...
class TestDispatch:
    ...
    def test_dispatch_unknown_command_exits(self, capsys):
        ...
        assert exc.value.code == 1
        assert "Unknown mesh command" in exc.value.message
        assert "culture mesh --help" in exc.value.remediation
...
class TestCollectMeshData:
    def _patch_collector(self, monkeypatch, *, side_effect=None, returns=None):
        """Patch `culture_core.overview.collector.collect_mesh_state` to a fake async."""
        ...
        monkeypatch.setattr("culture_core.overview.collector.collect_mesh_state", _fake)
    ...
    def test_timeout_exits_with_hint(self, monkeypatch, capsys):
        ...
        assert "culture server status" in exc.value.remediation
    ...
    def test_other_oserror_exits_with_message(self, monkeypatch, capsys):
        ...
        with pytest.raises(CultureError) as exc:
            mesh_mod._collect_mesh_data("127.0.0.1", 6667, "spark", 4)
        assert exc.value.code == 1
        assert "eperm" in exc.value.message
...
class TestCmdOverview:
    def test_non_serve_path_renders_text(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            "culture_core.overview.renderer_text.render_text",
    ...
    def test_messages_clamped(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "culture_core.overview.renderer_text.render_text", lambda mesh, **kw: ""
        )
    ...
    def test_serve_path_invokes_web(self, monkeypatch):
        ...
        monkeypatch.setattr(mesh_mod, "load_config_or_default", lambda _p: _StubFullConfig())
        monkeypatch.setattr(
            "culture_core.overview.renderer_web.serve_web",
...
class TestStoreMeshCredentials:
    def test_skips_when_credential_already_in_keyring(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("culture_core.credentials.lookup_credential", lambda n: "existing")
    ...
    def test_prompts_and_stores_when_missing(self, monkeypatch, capsys):
        mesh = _StubMesh(server=_StubServerCfg(links=[_StubLink(name="thor")]))
        monkeypatch.setattr("culture_core.credentials.lookup_credential", lambda n: None)
        monkeypatch.setattr("getpass.getpass", lambda _p: "secret")
        stored = []
        monkeypatch.setattr(
            "culture_core.credentials.store_credential",
    ...
    def test_warns_when_store_fails(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("culture_core.credentials.lookup_credential", lambda n: None)
        monkeypatch.setattr("getpass.getpass", lambda _p: "secret")
        monkeypatch.setattr("culture_core.credentials.store_credential", lambda *a: False)
...
class TestCmdSetup:
    def _setup(self, monkeypatch, *, mesh_load=None, generate=None):
        if mesh_load is not None:
            monkeypatch.setattr("culture_core.mesh_config.load_mesh_config", mesh_load)
    ...
    def test_uninstall_path(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(
            "culture_core.persistence.list_services",
            lambda: ["culture-server-spark", "culture-agent-spark-ada", "unrelated"],
        )
        removed = []
        monkeypatch.setattr("culture_core.persistence.uninstall_service", removed.append)
        stopped_servers = []
        stopped_agents = []
    ...
    def test_install_exits_when_no_mesh_and_generation_fails(self, monkeypatch, capsys):
        ...
        with pytest.raises(CultureError) as exc:
            mesh_mod._cmd_setup(_args(mesh_command="setup", uninstall=False))
        assert exc.value.code == 1
        assert "no mesh config" in exc.value.message
        assert "culture server start" in exc.value.remediation
...
class TestUpgradeTimeoutHint:
    ...
    def test_includes_direct_cmd_for_tool(self, tool, direct_cmd):
        """The remediation must offer all three recovery paths."""
        hint = mesh_mod._upgrade_timeout_hint(tool)
        assert direct_cmd in hint
        assert "--upgrade-timeout" in hint
...
class TestRunUpgrade:
    ...
    def test_timeout_exits_with_hint(self, monkeypatch, capsys):
        ...
        assert "--skip-upgrade" in exc.value.remediation
    ...
    def test_non_zero_exit_exits_with_error(self, monkeypatch, capsys):
        ...
        assert exc.value.code == 1
        assert "upgrade failed" in exc.value.message
        assert "--skip-upgrade" in exc.value.remediation
...
class TestUpgradeCulturePackage:
    ...
    def test_exits_when_no_upgrade_tool(self, monkeypatch, capsys):
        ...
        assert exc.value.code == 1
        assert "Neither uv nor pip" in exc.value.message
        assert "install uv" in exc.value.remediation
    ...
    def test_runs_upgrade_then_reexecs(self, monkeypatch):
        ...
        assert argv[:3] == [sys.executable, "-m", "culture_core"]
        assert "--skip-upgrade" in argv
...
class TestWaitForServerPort:
    ...
    def test_returns_false_when_pid_not_culture(self, monkeypatch):
        ...
        monkeypatch.setattr("culture_core.pidfile.is_culture_process", lambda _pid: False)
    ...
    def test_zero_pid_does_not_disqualify(self, monkeypatch):
        ...
        monkeypatch.setattr("culture_core.pidfile.is_culture_process", lambda _pid: True)
...
class TestRestartMeshServices:
    ...
    def test_returns_false_when_server_does_not_come_up(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr("culture_core.persistence.install_service", lambda *a, **kw: None)
        monkeypatch.setattr("culture_core.persistence.restart_service", lambda _svc: True)
...
class TestResolveMeshForServer:
    def test_returns_loaded_mesh_when_server_matches(self, monkeypatch):
        mesh = _StubMesh(server=_StubServerCfg(name="spark"))
        monkeypatch.setattr("culture_core.mesh_config.load_mesh_config", lambda _p: mesh)
    ...
    def test_returns_none_when_neither_path_matches(self, monkeypatch):
        ...
        monkeypatch.setattr("culture_core.mesh_config.load_mesh_config", _missing)
    ...
    def test_rebuilds_from_default_config(self, monkeypatch):
        ...
        monkeypatch.setattr("culture_core.mesh_config.from_daemon_config", lambda dc: rebuilt)
        monkeypatch.setattr("culture_core.mesh_config.merge_links", lambda *a, **kw: None)
        saved = []
        monkeypatch.setattr(
            "culture_core.mesh_config.save_mesh_config",
...
class TestCmdUpdate:
    ...
    def test_short_circuits_when_upgrade_returns_false(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "culture_core.pidfile.list_servers",
    ...
    def test_exits_with_error_on_failed_restart(self, monkeypatch, capsys):
        ...
        with pytest.raises(CultureError) as exc:
            mesh_mod._cmd_update(self._args_update())
        assert exc.value.code == 1
        assert "Failed to restart" in exc.value.message
        assert "--skip-upgrade" in exc.value.remediation
