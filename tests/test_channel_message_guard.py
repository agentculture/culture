...
from culture_core.cli import channel
...
def test_typo_channel_is_refused(isolate_ipc, monkeypatch, capsys):
    ...
    with pytest.raises(CultureError) as ei:
        channel._cmd_message(_args("#nonexistent"))

    assert ei.value.code == 1
    assert "#nonexistent" in ei.value.message
    assert "does not exist" in ei.value.message
    assert "--create" in ei.value.remediation  # hint to bootstrap if intentional
