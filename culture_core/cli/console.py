...
def _different_target_error(
    ...
    return CultureError(
        ...
        f"  Or stop it and start fresh: culture console stop --web-port {web_port}"
        f" && culture console {requested_server}\n"
        f"  Or run side-by-side: culture console {requested_server} --web-port {next_port}",
    )


def _foreign_irc_lens_error(web_port: int) -> CultureError:
    next_port = web_port + 1
    return CultureError(
        EXIT_USER_ERROR,
        f"port {web_port} is already serving an irc-lens instance, "
        "but it wasn't started by `culture console`",
        f"find the owner: ss -tlnp 'sport = :{web_port}' "
        f"(or: lsof -iTCP:{web_port} -sTCP:LISTEN) and stop that process manually, "
...
def _check_port_conflict(web_port: int, target: dict[str, Any]) -> None:
    ...
    3. **Slot owns the bound port but a different target** -> raise
       CultureError (exit 1) with a 3-option remediation.
    4. **Bound port is irc-lens but no slot points at it** -> raise
       CultureError (exit 1) with the foreign-irc-lens remediation.
    ...
    if _looks_like_irc_lens(web_port):
        raise _foreign_irc_lens_error(web_port)
...
def _cmd_stop(args: argparse.Namespace) -> None:
    ...
    # ``culture_core/cli/shared/process.py``.
...
def _invoke_irc_lens(argv: list[str]) -> "int | None":
    try:
        ...
        raise CultureError(
            EXIT_ENV_ERROR,
            f"irc-lens is not installed: {exc}",
            "reinstall the engine with its dependencies: uv sync (or pip install culture)",
        ) from exc
    return main(argv)
...
def _build_serve_argv(server_name: str | None, rest: list[str]) -> tuple[list[str], str | None]:
    ...
    if result is None:
        if server_name is None:
            raise CultureError(
                EXIT_USER_ERROR,
                "No culture servers running",
                "start one with: culture server start",
            )
        raise CultureError(
            EXIT_USER_ERROR,
            f"No culture server named {server_name!r}",
            "run 'culture server status' to see what's running",
        )
    name, port = result
