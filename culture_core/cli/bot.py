...
def _validate_name_or_raise(name: str) -> None:
    """Reject names that are not a safe single path segment (#19)."""
    from culture_core.bots.config import validate_bot_name

    if not validate_bot_name(name):
        raise CultureError(
            EXIT_USER_ERROR,
            f"invalid bot name '{name}'",
            "use a single path segment (no '/', '\\' or '..'), e.g. spark-ori-ghci; "
            "run 'culture bot list' to see existing bots",
        )


def _existing_bot_yaml(name: str) -> Path:
    """Return the bot's config path, raising with a remediation if absent."""
    from culture_core.bots.config import BOTS_DIR

    yaml_path = BOTS_DIR / name / BOT_CONFIG_FILE
    if not yaml_path.is_file():
        raise CultureError(
            EXIT_USER_ERROR,
            f"Bot '{name}' not found at {BOTS_DIR / name}",
            "run 'culture bot list' to see configured bots, or 'culture bot create' to add one",
        )
    return yaml_path
...
def dispatch(args: argparse.Namespace) -> None:
    ...
    if not handler:
        raise CultureError(
            EXIT_USER_ERROR,
            f"Unknown bot command: {args.bot_command}",
            "run 'culture bot --help' to list subcommands "
            "(create|start|stop|list|inspect|archive|unarchive)",
        )
    handler(args)
...
def _bot_create(args: argparse.Namespace) -> None:
    from culture_core.bots.config import BOTS_DIR, BotConfig, save_bot_config

    if not args.name.strip():
        raise CultureError(
            EXIT_USER_ERROR,
            "bot name cannot be empty",
            f"pass a bot name, e.g. culture bot create ghci --owner {args.owner}",
        )
    name = args.name
    ...
    if (bot_dir / BOT_CONFIG_FILE).exists():
        raise CultureError(
            EXIT_USER_ERROR,
            f"Bot '{name}' already exists at {bot_dir}",
            f"inspect it with 'culture bot inspect {name}', or choose a different name",
        )
...
def _bot_start(args: argparse.Namespace) -> None:
    _validate_name_or_raise(args.name)
    _existing_bot_yaml(args.name)
...
def _bot_inspect(args: argparse.Namespace) -> None:
    from culture_core.bots.config import load_bot_config

    _validate_name_or_raise(args.name)
    yaml_path = _existing_bot_yaml(args.name)
...
def _bot_archive(args: argparse.Namespace) -> None:
    ...
    from culture_core.bots.config import load_bot_config, save_bot_config
...
def _bot_unarchive(args: argparse.Namespace) -> None:
    ...
    if not config.archived:
        raise CultureError(
            EXIT_USER_ERROR,
            f"Bot '{args.name}' is not archived",
            "run 'culture bot list --all' to see archived bots",
        )
