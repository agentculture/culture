...
Extracted from ``culture_core.cli.mesh`` so both the new ``culture console``
...
from culture_core.pidfile import list_servers, read_default_server, read_port
...
def resolve_server(server_name: str | None) -> tuple[str, int] | None:
    ...
    """
    if server_name:
        # pid/port files are keyed `server-<name>` (matches culture_core.cli.server
