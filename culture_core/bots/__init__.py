"""culture_core.bots — the bot framework, now forwarded to ``agentirc.bots`` (issue #445).
...
``culture_core.bots.system`` (the welcome system bot), which agentirc does not
vendor.
...
registers ``culture_core.bots.system`` under that name in ``sys.modules`` so
...
def install_system_bridge() -> None:
    """Register ``culture_core.bots.system`` as ``agentirc.bots.system`` in ``sys.modules``.
    ...
    ``culture_core.bots`` installs it for every other path (e.g. the test fixtures
    that build a ``BotManager`` directly).
    """
    if "agentirc.bots.system" in sys.modules:
        return
    from culture_core.bots import system as _culture_system

    sys.modules["agentirc.bots.system"] = _culture_system


# Install on import so any code path that touches culture_core.bots (production or
