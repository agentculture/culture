...
dropping is preferable to blocking the event loop — but never silently:
`culture.audit.dropped{reason=queue_full|not_started}` increments
(distinct from `culture.audit.writes{outcome=error}`, which counts
write failures only) and `submit()` returns False so the caller sees
the backpressure. (#17)
...
if TYPE_CHECKING:
    ...
    from culture_core.telemetry.metrics import MetricsRegistry
...
class AuditSink:
    ...
    records silently (returning True — disabled is not backpressure).
    """
    ...
    def submit(self, record: dict) -> bool:
        ...
        Returns True when the record was enqueued (or the sink is
        disabled — not backpressure), False when it was dropped. A drop
        increments ``culture.audit.dropped`` with a ``reason`` attribute
        so overflow is distinguishable from write errors, and the False
        return surfaces the loss to the submitter without blocking. (#17)
        """
        if not self.enabled:
            return True
        if self.queue is None:
            ...
            self.metrics.audit_dropped.add(1, {"reason": "not_started"})
        ...
        try:
            ...
            self.metrics.audit_dropped.add(1, {"reason": "queue_full"})
