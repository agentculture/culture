...
from culture_core.formatting import relative_time as _relative_time
...
def _partial_snapshot_line(mesh: MeshState) -> str:
    """Describe partial collection: which rooms failed, and during which phase."""
    collected = len(mesh.rooms)
    total = collected + len(mesh.failed_rooms)
    failed_str = ", ".join(f"{fr.name} ({fr.error} during {fr.phase})" for fr in mesh.failed_rooms)
    return f"**Partial snapshot:** {collected} of {total} rooms collected; failed: {failed_str}"
...
def _render_default(mesh: MeshState, message_limit: int) -> str:
    ...
    if mesh.failed_rooms:
        parts.append("")
        parts.append(_partial_snapshot_line(mesh))
...
def _render_room_detail(mesh: MeshState, room_name: str, message_limit: int) -> str:
    ...
    for r in mesh.rooms:
        if r.name == room_name:
            ...
            break
    if room is None:
        for fr in mesh.failed_rooms:
            if fr.name == room_name:
                return f"Room {room_name} failed to collect ({fr.error} during {fr.phase}).\n"
